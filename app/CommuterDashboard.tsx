// app/Tricomplaint/CommuterDashboard.tsx
...
const CommuterDashboard = () => {
  ...
  const uploadAllFiles = async () => {
    ...
    for (const uri of localFiles) {
      ...
      const storageRef = ref(storage, `commuter/commuter1/${filename}`);
  ...
  const handleSubmit = async () => {
    if (!timeDate || !ticketNo || !destination || !violation) {
    ...
    try {
      ...
      await set(push(dbRef(database, "commuter/commuter1")), complainData);
